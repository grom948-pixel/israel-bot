...
def call_start():
    ...
    g.say(opening, language="he-IL", voice="Polly.Dina-Neural")
    vr.append(g)
...
def call_respond():
    ...
    if not data or not speech:
        vr.say("תודה רבה, שלום!", language="he-IL", voice="Polly.Dina-Neural")
        vr.hangup()
    ...
    if done or len(data["history"]) > 18:
        vr.say(clean, language="he-IL", voice="Polly.Dina-Neural")
        vr.say("תודה רבה, שלום!", language="he-IL", voice="Polly.Dina-Neural")
        vr.hangup()
        _finish(sid, data)
    else:
        g = Gather(input="speech", language="he-IL",
                   action=f"{SERVER_URL}/call/respond?sid={sid}",
                   timeout=6, speech_timeout="auto")
        g.say(clean, language="he-IL", voice="Polly.Dina-Neural")
        vr.append(g)
        vr.redirect(f"{SERVER_URL}/call/done?sid={sid}")
